...
def add_song_to_artist(artist_name: str, song_title: str) -> None:
    art_obj = Artist.objects.get(name=artist_name)
    song_obj = Song.objects.get(title=song_title)

    art_obj.songs.add(song_obj)
...
def calculate_licenses_expiration_dates() -> str:
    ...
    for license_obj in licenses:
        result.append(
            f'License with number: {license_obj.license_number} expires on {license_obj.issue_date + timedelta(days=365)}!'
        )
    return '\n'.join(result)
...
def get_drivers_with_expired_licenses(due_date: date) -> QuerySet[Driver]:
    return Driver.objects.filter(
        license__issue_date__gt=due_date - timedelta(days=365)
    )
