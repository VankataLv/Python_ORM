...
# ----------- 1 ----------------
def find_books_by_genre_and_language(genre_arg, language_arg) -> QuerySet:
    return Book.objects.filter(genre=genre_arg, language=language_arg)
...
def find_authors_nationalities() -> str:
    return '\n'.join([f"{str(a)} is {a.nationality}" for a in Author.objects.exclude(nationality=None)])
...
# ----------- 4 ----------------
def delete_review_by_id(id_arg) -> str:
    review = Review.objects.get(id=id_arg)
    review.delete()
...
# ----------- 5 ----------------
def filter_authors_by_nationalities(nat_arg) -> str:
    authors = Author.objects.filter(nationality=nat_arg).order_by('first_name', 'last_name')
    result = []
    for a in authors:
        if a.biography == "" or a.biography is None:
            result.append(str(a))
        else:
...
def filter_authors_by_birth_year(first_year, second_year) -> str:
    authors = Author.objects.filter(birth_date__year__gte=first_year,
                                    birth_date__year__lte=second_year).order_by('-birth_date')
...
# ----------- 7 ----------------
def change_reviewer_name(original_name:str, new_name:str) -> QuerySet:
    Review.objects.filter(reviewer_name=original_name).update(reviewer_name=new_name)
